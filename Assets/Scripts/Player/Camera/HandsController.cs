...
public class HandsController : MonoBehaviour
{
    ...
    private void UpdateInput()
    {
        ...
        /*
        if (playerControls.Interactions.Reload.WasPerformedThisFrame() && CanReload_Internal())
        {
            StopActiveCoroutines(allowWeaponChangeRoutine: false);
    ...
    private void UpdateActivePoses()
    {
        ...
        if (playerMovement != null && playerMovement.IsRunningAndMoving())
            currentHandsPose = heldItem.RunPose;
    }

    private void UpdateMovementBounce(float deltaTime)
    {
        if (playerMovement == null || heldItem == null || currentHandsPose == null)
            return;
        ...
        float moveVelocityMagnitude = playerMovement.GetMoveVelocityMagnitude();
